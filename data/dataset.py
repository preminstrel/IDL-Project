...
def get_dataloader(config, tokenizer):
    if config["dataset"] == "twitter_complaints":
        ...
        classes = [
            k.replace("_", " ") for k in dataset["train"].features["Label"].names
        ]
        dataset = dataset.map(
        ...
        def collate_fn(batch):
            input_ids = torch.stack(
                [torch.tensor(item["input_ids"]).squeeze(0) for item in batch]
            )
            attention_mask = torch.stack(
                [torch.tensor(item["attention_mask"]).squeeze(0) for item in batch]
            )
